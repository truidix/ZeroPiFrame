...
def get_last_update() -> dict | None:
    """Reads the outcome of the last "Install update" run (written by
...
SOURCE_DIR_FILE = Path('/opt/zeropiframe/.source_dir')

# Same trailer convention update.sh (generated by install.sh) checks
# authoritatively before deciding deploy vs. full install - this copy is
# only used to preview that decision in the "Check for updates" result
# (see api_check_update()). Keep in sync with the grep in install.sh.
_FULL_INSTALL_TRAILER_RE = re.compile(
    r'^Requires-Install:\s*full\s*$', re.IGNORECASE | re.MULTILINE)


def _repo_dir() -> Path | None:
    """Where the git checkout install.sh was run from lives - written by
    install.sh into SOURCE_DIR_FILE (both the full install and --deploy
    paths). None if that file doesn't exist (e.g. a very old install from
    before this existed, or the recorded path is no longer a real git
    checkout - moved or deleted since)."""
    try:
        repo_dir = Path(SOURCE_DIR_FILE.read_text().strip())
    except Exception:
        return None
...
def api_check_update():
    """Read-only: fetches from the git remote and reports whether the
    checkout install.sh was run from (see _repo_dir()) is behind, along
    with the most recent commit's subject line and a preview of whether
    installing it would need a deploy or a full install (see
    _FULL_INSTALL_TRAILER_RE - the authoritative version of this same
    check lives in update.sh itself, run again at install time).

    Doesn't touch the installed copy in /opt/zeropiframe or go through
    sudo at all - zeropiframe-webui.service already runs as the same
    unprivileged user that owns the checkout (see SETUP.md), so plain
    git commands here have everything they need without any of the
    privileges update.sh requires for the actual install.
    """
    t = i18n.make_translator(current_language())
    repo_dir = _repo_dir()
    if repo_dir is None:
        return jsonify({'ok': False, 'message': t('api.update_no_source_dir')})
    ...
    try:
        ...
        if upstream == head:
            return jsonify({'ok': True, 'behind': False})

        commit_count   = int(git('rev-list', '--count', f'HEAD..{upstream}'))
        latest_message = git('log', '-1', upstream, '--format=%s')
        # %B (not %s) - the trailer this looks for lives in the commit
        # body, not the subject line.
        full_bodies    = git('log', f'HEAD..{upstream}', '--format=%B')
        mode = 'full' if _FULL_INSTALL_TRAILER_RE.search(full_bodies) else 'deploy'
        ...
        log.warning(f'check_update failed: {e}')
        return jsonify({'ok': False, 'message': t('api.update_check_failed')})


@app.route('/api/update', methods=['POST'])
def api_update():
    """Triggers the actual update - see update.sh (generated by
    install.sh) via the tightly scoped sudoers rule set up for that
    purpose. Takes no arguments: update.sh decides deploy vs. full
    install entirely on its own (by re-checking the same
    Requires-Install: full trailer convention right after pulling - see
    ...
    *started*; write an "in progress" status right away so a concurrent
    ...
    with the real outcome via LAST_UPDATE_PATH once it's done - polled by
    the page afterwards through /api/update_status, after the restarted
    webui is back up to answer it.
    """
    t = i18n.make_translator(current_language())
    ...
    try:
        ...
        LAST_UPDATE_PATH.write_text(json.dumps({
            'ok': None,
            'message': t('api.update_in_progress'),
    ...
    return jsonify({'ok': True, 'message': t('api.update_started')})
