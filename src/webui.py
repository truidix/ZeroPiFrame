...
def api_update():
    """Triggers "git pull" + a deploy (see update.sh, generated by
    ...
    the deploy step restarts zeropiframe-webui itself partway through,
    which would otherwise tear down the very process handling this
    request before it could send a response. This response only confirms
    the update *started*; write an "in progress" status right away so a
